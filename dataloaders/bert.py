...
class BertDataloader(AbstractDataloader):
    def __init__(self, args, dataset):
        ...
        self.train_slidewindow, self.train_slidewindow_by_user, self.user_count_slidewindow = self.get_train_dataset_slidewindow(
            args.slide_window_step)
    ...
    def _get_train_dataset(self):
        ...
        dataset = BertTrainDataset(self.train_slidewindow, self.num_positive, self.max_len, self.mask_prob,
...
class RecWithContrastiveLearning():
    def __init__(self, args, MASK_TOKEN):
        ...
        self.augmentations = {'truncation': Truncation(truncation_rate=args.truncation_rate),
                              ...
                              'combinatorial_enumerate': CombinatorialEnumerate(truncation_rate=args.truncation_rate,
                                                                                gamma=args.mask_rate,
                                                                                beta=args.reorder_rate,
...
class BertTrainDataset(data_utils.Dataset):
    ...
    def get_unmasked_seq(self, seq):
        unmasked_tokens = []
        for s in seq:
            unmasked_tokens.append(s)
        unmasked_tokens = unmasked_tokens[-self.max_len:]
        mask_len = self.max_len - len(unmasked_tokens)
        unmasked_tokens = [0] * mask_len + unmasked_tokens
    ...
    def __getitem__(self, index):
        ...
        '''
        return_list = []
        unmasked_tokens = self.get_unmasked_seq(seq)
        ...
        return  index, tuple(return_list),torch.tensor(unmasked_tokens)
...
class BertTestDataset(data_utils.Dataset):
    ...
    def __getitem__(self, index):
        ...
        seq = [0] * padding_len + seq
        interacted = set(answer + seq)  # remove items that user has interacted with as candidate items
        candidates = answer + [x for x in range(1, self.num_items + 1) if
                               x not in interacted]  # rank on the whole item set
