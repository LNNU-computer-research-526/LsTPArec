...
class NTXENTloss(nn.Module):
    ...
    def forward(self, h1, h2, calcsim='dot'):
        ...
        if self.args.projectionhead:
            z1, z2 = self.project(h1.view(b * self.args.bert_max_len, self.args.bert_hidden_units)), self.project(
                h2.view(b * self.args.bert_max_len, self.args.bert_hidden_units))
        else:
            z1, z2 = h1, h2
        z1 = z1.reshape(b, self.args.bert_max_len * self.args.bert_hidden_units)
        z2 = z2.reshape(b, self.args.bert_max_len * self.args.bert_hidden_units)
        if calcsim == 'dot':
...
class BERTTrainer(AbstractTrainer):
    ...
    def calculate_loss(self, batch, enbale_DA=False):
        ...
        if enbale_DA:
            ...
            proportion = self.alpha * loss.detach().data.item() / (
                    loss.detach().data.item() + cl_loss.detach().data.item())
            ...
            for i in range(num_positive):
                ...
                logits_k, c_i_k= self.model([seqs, un_mask])
